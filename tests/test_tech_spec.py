...
class TestTechSpec(unittest.TestCase):
    ...
    def test_tech_spec_json(self):
        ...
        self.assertTrue(os.path.exists("tech_spec.json"), "Output file tech_spec.json was not created")

        self.assertTrue(
            filecmp.cmp("tech_spec.json", expected_output), "Files are not identical"
        )

        if not filecmp.cmp("tech_spec.json", expected_output):
            subprocess.run(["diff", expected_output, "tech_spec.json"], check=True)
    ...
    def test_tech_spec_xml(self):
        ...
        self.assertTrue(os.path.exists("tech_spec.xml"), "Output file tech_spec.xml was not created")

        self.assertTrue(
            filecmp.cmp("tech_spec.xml", expected_output), "Files are not identical"
        )

        if not filecmp.cmp("tech_spec.xml", expected_output):
            subprocess.run(["diff", expected_output, "tech_spec.xml"], check=True)
    ...
    def tearDown(self):
        ...
        if os.path.exists("b.html"):
            os.remove("b.html")
        if os.path.exists("tech_spec.json"):
            os.remove("tech_spec.json")
        if os.path.exists("tech_spec.xml"):
            os.remove("tech_spec.xml")
        if os.path.exists("tech_spec.csv"):
            os.remove("tech_spec.csv")
